...
void CanonCamera::DownloadImage(EdsDirectoryItemRef directoryItem, EdsChar *path)
{
	  ...
	  EdsChar fName[EDS_MAX_NAME+1];
	  sprintf_s(fName, "%s", path);
	  if (path[strlen(path)-1] != '\\')
	  {
		  strcat_s(fName, EDS_MAX_NAME, "\\");
	  }
	  strcat_s(fName, EDS_MAX_NAME, dirItemInfo.szFileName);

	  if(err == EDS_ERR_OK)
	  {
		  //err = EdsCreateFileStream( dirItemInfo.szFileName,
		  err = EdsCreateFileStream( fName,
	  ...
	  if (strstr(dirItemInfo.szFileName, ".CR2") <= 0)
	  {
		  return;
	  }
	  err = EdsCreateFileStream(fName, kEdsFileCreateDisposition_OpenExisting, kEdsAccess_Read, &stream);
	  if( stream != NULL)
	  {
		  EdsImageRef srcImg;
		  EdsStreamRef dstStream = NULL;
		  EdsSaveImageSetting imgQ;
		  imgQ.JPEGQuality = 10;
		  imgQ.iccProfileStream = NULL;
		  if(err == EDS_ERR_OK)
		  {
			  err = EdsCreateImageRef(stream, &srcImg);
	
			  strcat_s(fName, EDS_MAX_NAME, ".jpg");			  
			  EdsStreamRef DstStreamRef;
			  EdsError err = EdsCreateFileStream( fName , kEdsFileCreateDisposition_CreateAlways , 
													kEdsAccess_Write , &DstStreamRef );
			  if( err == EDS_ERR_OK )
			  {
				  /// Picture preservation is performed.
				  err = EdsSaveImage( srcImg , kEdsTargetImageType_Jpeg , imgQ , DstStreamRef );
			  }
											...
											EdsVoid * context)
{
	...
	switch(evt)
	{
		...
		case kEdsObjectEvent_DirItemRequestTransferDT:
		{
			...
			if (context != NULL)
			{
				sprintf_s(path, "%s", (EdsChar*)context);
			}
			else
			{
				sprintf_s(path, ".\\");
